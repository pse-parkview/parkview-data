...
def get_branches(repo: git.Repo) -> Iterable[git.refs.reference.Reference]:
    return filter(lambda x: x.name.replace("origin/", "") != "HEAD", repo.remote().refs)
...
def main():
    ...
    branches = list(get_branches(repo))
